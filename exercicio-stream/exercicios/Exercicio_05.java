...
// Listar as pessoas que são do Brasil, em ordem de nome
    List<Pessoa> pessoasDoBrasil = pessoas.stream().filter(p -> p.getPais().equals("Brazil")).sorted(Comparator.comparing(Pessoa::getNome)).collect(Collectors.toList());;
        pessoasDoBrasil.forEach(p2 -> System.out.println(p2.getNome()));
